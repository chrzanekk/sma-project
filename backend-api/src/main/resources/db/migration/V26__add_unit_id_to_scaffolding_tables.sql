...
SET scaffolding_full_dimension_unit_id = (SELECT id
                                          FROM units
                                          WHERE symbol = 'm2'
...
-- Sprawdź czy wszystkie rekordy zostały zaktualizowane
DO $$
DECLARE
    null_count INTEGER;
    total_count INTEGER;
BEGIN
    ...
    IF null_count > 0 THEN
        RAISE EXCEPTION 'Found % out of % scaffolding_log_position records without unit_id. Global m3 unit not found!', null_count, total_count;
    END IF;

    RAISE NOTICE 'Successfully updated % scaffolding_log_position records with m3 unit', total_count;
END $$;
...
-- =============================================================================
COMMENT
ON COLUMN scaffolding_log_position.scaffolding_full_dimension_unit_id IS 'Measurement unit for total scaffolding dimension (m2 or m3) - determined by aggregated dimensions';
COMMENT
ON COLUMN scaffolding_log_position_dimensions.unit_id IS 'Measurement unit (m2, m3, mb) - determined by width value';
COMMENT
ON COLUMN scaffolding_log_position_working_time.unit_id IS 'Measurement unit for working time (r-h - roboczogodzina)';
