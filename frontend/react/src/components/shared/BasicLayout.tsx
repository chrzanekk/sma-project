...
const BasicLayout: React.FC<BasicLayoutProps> = ({
    ...
    return (
        ...
        >
            ...
            {filters && <GridItem w="100%" p={1} bg={layoutBgColor} borderRadius="lg">
                <Heading size="sm" fontSize={14} textAlign="center" color={themeColors.fontColor}>
                    {t("filters")}
                </Heading>
                {filters}
            </GridItem>}
            {addButton && <GridItem w="100%" bg={layoutBgColor} p={1} borderRadius="lg">
                {addButton}
            </GridItem>}
            <GridItem w="100%" borderRadius="lg" overflowY="auto" p={2}>
                {table}
            </GridItem>
            {pagination && <GridItem>{pagination}</GridItem>}
        </Grid>
    );
};
