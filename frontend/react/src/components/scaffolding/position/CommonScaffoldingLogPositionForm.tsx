...
import {fetchNextBaseScaffoldingNumber} from "@/services/scaffolding-log-position-service.ts";
import {getNextChildSuffix} from "@/utils/scaffolding-number-util.ts";
...
const FormContent: React.FC<{ disabled: boolean, hideSubmit: boolean }> = (({
        ...
        useEffect(() => {
            async function updatePreviewNumber() {
                if (!debouncedAssemblyDate || values.id) {
                    return; // Nic nie robimy jeśli edytujemy (values.id) lub nie ma daty
                }

                const [year, month, day] = debouncedAssemblyDate.split('-');
                let newScaffoldingPrefix = "";

                if (values.parentPosition) {
                    // WARUNEK DLA DZIECKA (robione na żywo, czysto UI)
                    const parentPrefixStr = values.parentPosition.scaffoldingNumber.split('/')[0];
                    const parentBaseNumber = parentPrefixStr.replace(/[a-zA-Z]/g, '');
                    const nextSuffix = getNextChildSuffix(values.parentPosition.childPositions || []);
                    newScaffoldingPrefix = `${parentBaseNumber}${nextSuffix}`;

                } else if (values.scaffoldingLog?.id) {
                    // WARUNEK DLA RODZICA (odpytanie API "peek")
                    try {
                        // To nie zapisuje do bazy, tylko podgląda
                        const nextBaseNumber = await fetchNextBaseScaffoldingNumber(
                            values.scaffoldingLog.id, parseInt(year)
                        );
                        newScaffoldingPrefix = `${nextBaseNumber}`;
                    } catch (error) {
                        console.error("Błąd podczas przewidywania numeru: ", error);
                    }
                }

                if (newScaffoldingPrefix) {
                    const finalScaffoldingNumber = `${newScaffoldingPrefix}/${day}/${month}/${year}`;
                    setFieldValue('scaffoldingNumber', finalScaffoldingNumber).then();
                }
            }

            updatePreviewNumber().then();
            // Nasłuchujemy zmiany zdobouncowanej daty, logu i rodzica
        }, [debouncedAssemblyDate, values.parentPosition, values.scaffoldingLog?.id, setFieldValue, values.id]);
                            ...
                            <Grid templateColumns="repeat(6, 1fr)" gap={4}>
                                ...
                                <GridItem colSpan={4}>
                                    <CustomInputField
                                        ...
                                        label={t('scaffoldingLogPositions:scaffoldingNumberAutoGenerated')}
                                        placeholder={t('scaffoldingLogPositions:scaffoldingNumberAutoGenerated')}
                                        disabled={true}
                                    />
                                </GridItem>
                                <GridItem colSpan={2}>
